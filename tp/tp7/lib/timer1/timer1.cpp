...
void Timer1::setDuration(uint16_t durationA_ms, uint16_t durationB_ms)
{
...
void Timer1::setDurationA(uint16_t durationA_ms)
{
	setOCR1A(Util::convertMsToCycles(durationA_ms, clock_));
	if (OCR1A_cycle_ <= TCNT1)
	{
...
void Timer1::setDurationB(uint16_t durationB_ms)
{
	setOCR1B(Util::convertMsToCycles(durationB_ms, clock_));
	OCR1B = OCR1B_cycle_;
}
