...
Piezo::Piezo(Pin& groundPin, Pin& signalPin, Timer1& timer)
...
{
    float noteFrequency = convertNoteToFrequency(note); // Hertz
    timer_->setDurationAUs(Util::ONE_SECOND_US / noteFrequency);
    timer_->startTimer();
}
